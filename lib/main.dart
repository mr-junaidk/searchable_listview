      ...
      body: Container(
        ...
        child: _foundedUsers.length > 0 ? ListView.builder(
            ...
            itemBuilder: (context, index) {
              return Slidable(
                // Use SlidableActionPane instead of SlidableDrawerActionPane
                key: ValueKey(_foundedUsers[index]), // A unique key is required for each slidable item
                startActionPane: ActionPane(
                  motion: const DrawerMotion(), // Use DrawerMotion for a drawer-like motion
                  extentRatio: 0.25, // Action extent ratio
                  children: [
                    SlidableAction(
                      onPressed: (context) => print("Archive"),
                      backgroundColor: Colors.transparent,
                      foregroundColor: Colors.black, // Specify foreground color for icons/text
                      icon: Icons.archive,
                      label: 'Archive',
                    ),
                    SlidableAction(
                ...
                child: userComponent(user: _foundedUsers[index]), // Your custom user component
              );
